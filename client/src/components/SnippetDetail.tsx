...
const SnippetDetail: React.FC = () => {
  ...
  const { loading, error, data } = useQuery<GetSnippetData, GetSnippetVariables>(
    GET_SNIPPET,
    {
      variables: { id: id! },
      skip: !id,
  ...
  const [deleteSnippet, { loading: deleting }] = useMutation<DeleteSnippetData, DeleteSnippetVariables>(
    DELETE_SNIPPET,
    {
      onCompleted: () => {
        navigate('/');
        ...
        try {
          ...
          if (existingData) {
            cache.writeQuery({
              ...
              variables: {},
              data: {
                snippets: existingData.snippets.filter((snippet: any) => (snippet as any).id !== id),
              },
            });
          }
        } catch {
